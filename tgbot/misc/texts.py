...
START_REQUEST_MESSAGE = (
        ...
        '\n' +
        hitalic(
            '- бачите пересування ворогів, підозрілі предмети - повідомляйте до телеграм боту:'
            '\n'
            'Знайден Ворог (@znaydenvorogbot);' +
            DOUBLE_NEW_LINE +
            '- для повідомлення підрозділів Національної Поліції України '
            'про особливі позначки на дорогах та контролю за '
            'пересуваннями загарбницьких сил РФ - Народный месник (@ukraine_avanger_bot);' +
            DOUBLE_NEW_LINE +
            '- снаряди, що не розірвались - Тарас бот (@bomb_found_bot);'
        ) +
        '\n'
)
